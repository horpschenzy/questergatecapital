...
<html class="bg-gray-200"><head>
    ...
    <title>Admin Withdrawal Lists - QuesterGate Limited</title>
    @include('favicon')
                ...
                <main class="p-6 flex-1 bg-gray-200 overflow-y-auto relative">
                    ...
                    <div class="md:flex">
                        <div class="w-full md:w-5/6 ">                            
                            <span class="text-xl md:text-2xl leading-snug font-semibold tracking-tight text-gray-900">Withdrawal Lists</span>
                        </div>

                        <div class="w-full md:w-1/6">
                            <div class="relative">
                                @if(Auth::user()->usertype == 'superadmin')
                                    <a href="{{ url('admin/export/withdrawals') }}"><button type="button" class="text-white px-4 py-2 font-semibold bg-lime-100 border rounded focus:outline-none focus:shadow-outline w-full withdraw-fund""><span>Export Withdrawals</span></button></a>
                                @endif
                            </div>
                        </div>
                        ...
                        <table class="w-full whitespace-no-wrap withdrawal-list">
                            ...
                            @if($d->amount != 0.00)
                                <tr class="hover:bg-gray-100 focus-within:bg-gray-100 tr-data">
                                    ...
                                    <td class="border-t">
                                        <a onclick="changeWithdrawalStatusPrompt(this);" data-id="{{$d->id}}" data-value="{{$d->status}}"><span tabindex="-1" class="items-center">Change Status</span></a>
                                    </td>
                                </tr>
                            @endif 
                            @endforeach
                        </table>
                        <div class="p-6"> {!! $withdrawal->links() !!} </div>
                    </div> 
