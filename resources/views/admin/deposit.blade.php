        ...
        <div class="h-screen flex">
            ...
            <div class="flex-1 min-w-0 flex flex-col bg-white">
                ...
                <main class="p-6 flex-1 bg-gray-200 overflow-y-auto">
                    <h2 class="text-xl md:text-2xl leading-snug font-semibold tracking-tight text-gray-900">Deposit</h2>
                        <form  method="POST" class="user-deposit-form">
                            <div class="deposit-response"></div>
                            <div>
                                <label>Status</label>
                                <select name="status" class="bg-white text w-full focus:outline-none px-3 py-3 rounded text-gray-900 border focus:bg-white mt-2 border-gray-400">
                            ...
                            <button type="button" onclick="createDeposit(this)" class="focus:outline-none focus:shadow-outline w-full md:flex-1 bg-red-500 hover:bg-red-600 px-4 py-3 text-white rounded border-b-2 border-red-500 font-semibold mt-4">Save</button>
                        </form>
...
var _urlpath = window.location.host != "earnaya.com" ? "/earnaya-ui" : "/app";
    ...
    function createDeposit(e){
      ...
      var ajaxPost = $.ajax({
         ...
         data:$(".user-deposit-form").serialize(),
         dataType: "json",
      ...
      ajaxPost.done(function(res){
        $(e).html("Send");
          if(res.message == "success"){
            $(".deposit-response").html('<div style="color: green"><strong>Deposit sent successfully.</strong></div>');

            window.location.href = _protocol+""+_shost+"/admin/deposit/list";
          }
          else{
            $(".deposit-response").html('<div style="color: red"><strong>'+res.message+'</strong></div>');
          }
      });
