        ...
        <div class="h-screen flex">
            ...
            <div class="flex-1 min-w-0 flex flex-col bg-white">
                ...
                <main class="p-6 flex-1 bg-gray-200 overflow-y-auto">
                    <h2 class="text-xl md:text-2xl leading-snug font-semibold tracking-tight text-gray-900">Withdrawal</h2>
                        <form  method="POST" class="user-withdrawal-form">
                            <div class="withdrawal-response"></div>
                            

                            <div class="mt-5">
                                <label>Status</label>
                                <input type="number" name="status" class="bg-white text w-full focus:outline-none px-3 py-3 rounded text-gray-900 border focus:bg-white mt-2 border-gray-400">
                            </div>
                            ...
                            <button type="button" onclick="updateWithdrawal(this)" class="focus:outline-none focus:shadow-outline w-full md:flex-1 bg-red-500 hover:bg-red-600 px-4 py-3 text-white rounded border-b-2 border-red-500 font-semibold mt-4">Save</button>
                        </form>
...
var _urlpath = window.location.host != "earnaya.com" ? "/earnaya-ui" : "/app";
    ...
    function updateWithdrawal(e){
      $(e).html("Sending...");

      var ajaxPost = $.ajax({
         type:'PUT',
         url:_protocol+""+_shost+"/admin/user/withdrawal/update",
         data:$(".user-withdrawal-form").serialize(),
         dataType: "json",
      ...
      ajaxPost.done(function(res){
        $(e).html("Send");
          if(res.message == "success"){
            $(".withdrawal-response").html('<div style="color: green"><strong>Withdrawal updated successfully.</strong></div>');

            window.location.href = _protocol+""+_shost+"/admin/withdrawal/list";
          }
          else{
            $(".withdrawal-response").html('<div style="color: red"><strong>'+res.message+'</strong></div>');
          }
      });
